...
// Equivalent for active variables
typedef struct {
  adouble rho,c;
} aField;
...
int main(int argc,char **argv)
{
  ...
  ierr = PetscOptionsGetScalar(NULL,NULL,"-delta",&appctx.delta,NULL);CHKERRQ(ierr);
...
//  PetscScalar    rho,c,rhoxx,cxx,cx,rhox,kcxrhox;
  ...
  */
  ...
  aField   u_a[xs+xm];				// independent variables
  aField   f_a[xs+xm];				// dependent variables
  adouble  rho,c,rhoxx,cxx,cx,rhox,kcxrhox;	// temporaries
  ...
  if (!xs) {
    f_a[0].rho = udot[0].rho; /* u[0].rho - 0.0; */
    f_a[0].c   = udot[0].c; /* u[0].c   - 1.0; */
    xs++;
    xm--;
  }
  if (xs+xm == Mx) {
    f_a[Mx-1].rho = udot[Mx-1].rho; /* u[Mx-1].rho - 1.0; */
    f_a[Mx-1].c   = udot[Mx-1].c;  /* u[Mx-1].c   - 0.0;  */
    xm--;
  }
  ...
  */
  for (i=xs; i<xs+xm; i++) {
    rho   = u_a[i].rho;
    rhoxx = (-2.0*rho + u_a[i-1].rho + u_a[i+1].rho)*sx;
    c     = u_a[i].c;
    cxx   = (-2.0*c + u_a[i-1].c + u_a[i+1].c)*sx;

    if (!appctx->upwind) {
      rhox    = .5*(u_a[i+1].rho - u_a[i-1].rho)/hx;
      cx      = .5*(u_a[i+1].c - u_a[i-1].c)/hx;
      kcxrhox = appctx->kappa*(cxx*rho + cx*rhox);
    } else {
      kcxrhox = appctx->kappa*((u_a[i+1].c - u_a[i].c)*u_a[i+1].rho - (u_a[i].c - u_a[i-1].c)*u_a[i].rho)*sx;
    }

    f_a[i].rho = udot[i].rho - appctx->epsilon*rhoxx + kcxrhox  - appctx->mu*PetscAbsScalar(rho)*(1.0 - rho)*PetscMax(0,PetscRealPart(c - appctx->cstar)) + appctx->beta*rho;
    //f_a[i].rho = udot[i].rho - appctx->epsilon*rhoxx + kcxrhox  - appctx->mu*rho*(1.0 - rho)*PetscRealPart(c - appctx->cstar) + appctx->beta*rho;
    f_a[i].c   = udot[i].c - appctx->delta*cxx + appctx->lambda*c + appctx->alpha*rho*c/(appctx->gamma + c);
  }
