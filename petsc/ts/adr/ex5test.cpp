  ...
  if (!appctx->no_an) {
    ...
    for (j=ys; j<ys+ym; j++) {
      for (i=xs; i<xs+xm; i++) {
        ...
        // Compute function over the locally owned part of the grid
        uc          = appctx->u_a[j][i].u;
        uxx         = (-2.0*uc + appctx->u_a[j][modulo(i-1,Mx)].u + appctx->u_a[j][modulo(i+1,Mx)].u)*sx;
        uyy         = (-2.0*uc + appctx->u_a[modulo(j-1,My)][i].u + appctx->u_a[modulo(j+1,My)][i].u)*sy;
        vc          = appctx->u_a[j][i].v;
        vxx         = (-2.0*vc + appctx->u_a[j][modulo(i-1,Mx)].v + appctx->u_a[j][modulo(i+1,Mx)].v)*sx;
        vyy         = (-2.0*vc + appctx->u_a[modulo(j-1,My)][i].v + appctx->u_a[modulo(j+1,My)][i].v)*sy;
